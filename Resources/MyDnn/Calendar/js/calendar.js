...
Calendar.dayMouseDown = function (ev) {
    ...
    //mydnn clear button click
    if (el.navtype == 367) {
        var mydnndatePicker = $find(cal.params['telerikDatePicker'].id);
        mydnndatePicker.clear();

        $("#" + cal.params["inputField"].id).val("");
        $(cal.element).fadeOut(250);
...
Calendar.cellClick = function (el, ev) {
    ...
    if (typeof el.navtype == "undefined") {
        ...
        if (cal.params['telerikDatePicker'] != null) {
            var mydnndatePicker = $find(cal.params['telerikDatePicker'].id);
            mydnndatePicker.set_selectedDate(date);
        }
...
Calendar.prototype.create = function (_par) {
    ...
    var hh = function (text, cs, navtype) {
        ...
        if (mydnnclass == "") Calendar._add_evs(cell);

        cell.calendar = cal;
        cell.navtype = navtype;


        cell.innerHTML = "<div unselectable='on' class='" + mydnnclass + "'>" + text + "</div>";
    ...
    this._nav_now.className = "mydnnToday";

    this._mydnnclear = hh("خالی", 2, 367);
    this._mydnnclear.ttip = "خالی نمودن تاریخ";
    this._mydnnclear.className = "mydnnToday";
    ...
    var title_length = 3;
    if (this.isPopup) title_length = 3;
    if (this.weekNumbers) title_length = 4;
...
Calendar.prototype.setDate = function (date) {
    if (this.params['telerikDatePicker'] != null) {
        var mydnndatePicker = $find(this.params['telerikDatePicker'].id);
        mydnndatePicker.set_selectedDate(date);
    }
...
Calendar.prototype.showAt = function (x, y) {
    ...
    x -= 48;
    y += 5;
    y = $("#" + this.params['button'].id).offset().top + 15;
    s.left = x + "px";
    s.top = y + "px";
    this.show();
};
